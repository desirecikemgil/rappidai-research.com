...
export default function LicensingPage() {
  const content = licensingContent;
    ...
    <>
      ...
      <section className="page-shell pb-[var(--section-space)]">
        <Reveal className="liquid-surface">
          <div
            role="region"
            aria-label="Scrollable licensing matrix"
            tabIndex={0}
          ...
          >
            <table className="w-full min-w-[72rem] border-collapse text-left">
              ...
              <thead>
                <tr>
                  {["Material", "Status", "Current terms", "Boundary"].map(
                    (column) => (
                      <th
                        key={column}
                        scope="col"
        ...
        <Reveal className="mt-8 border-l-2 border-accent bg-pale-soft px-6 py-5">
          <p className="max-w-[64rem] text-sm leading-6 text-ink-soft">
            This is a project-maintenance record, not legal advice. No
            unresolved material is described as open source, open weight or
            freely reusable.
          </p>
          <a
      ...
      <ResourceDirectory current="licensing" />
    </>
  );
}
