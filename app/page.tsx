...
const diagramKinds = ["adaptation", "inference", "evaluation"] as const;
    ...
    <>
        ...
        <div className="page-shell-wide grid min-h-[calc(100svh-var(--header-height))] items-center gap-4 py-[clamp(4.5rem,8vw,7.5rem)] lg:grid-cols-[1.02fr_0.98fr]">
          <div className="liquid-hero-copy relative z-10 max-w-[47rem]">
            ...
            <Reveal delay={0.36} className="mt-9 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
              ...
              {heroExternal.url ? (
                <ActionLink href={heroExternal.url} external variant="secondary">
          ...
          <Reveal delay={0.12} distance={10} className="relative -mr-[8%] hidden lg:block">
            <HeroVisualization />
          </Reveal>
        </div>
      </section>
          ...
          <div className="liquid-surface mt-8 grid gap-12 p-7 py-12 sm:p-10 lg:grid-cols-[0.45fr_0.55fr] lg:gap-20 lg:p-14 lg:py-16">
            ...
            <Reveal delay={0.08}>
              ...
              <dl className="mt-10 border-t border-line">
                <FeaturedFact label="Model type" value={featuredModel.modelType} />
                <FeaturedFact label="Primary use" value={featuredModel.intendedUse[0]} />
                <FeaturedFact label="Languages" value={featuredModel.languages.join(" and ") + " experimentation"} />
                <FeaturedFact label="Release" value={featuredModel.releaseStatus} />
              </dl>
      ...
      <section className="bg-ink text-white">
        <div className="page-shell section-space">
          <div className="grid gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:items-end">
            <Reveal>
              <p className="eyebrow text-[#79aaff]">{homePageContent.modelEvolution.eyebrow}</p>
            </Reveal>
            <Reveal delay={0.06}>
              <h2 className="display-section text-white">{homePageContent.modelEvolution.title}</h2>
            </Reveal>
          </div>
          ...
          <div className="relative mt-16 grid gap-4 lg:grid-cols-3">
            ...
            {experimentLogs.map((entry, index) => (
              ...
              >
                ...
                <p className="mt-5 max-w-sm leading-7 text-[#b9c7dc]">{entry.description}</p>
              </Reveal>
            ))}
          </div>
        </div>
      ...
      <section className="page-shell section-space">
        <div className="grid gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:items-end">
          ...
          <Reveal delay={0.06}>
            <h2 className="display-section">{homePageContent.currentResearch.title}</h2>
          </Reveal>
        </div>
      ...
      <section className="liquid-section border-y border-line bg-pale-soft/40">
        <div className="page-shell section-space-sm grid gap-12 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
          <Reveal>
            ...
            <h2 className="display-section mt-7">{homePageContent.openResearch.headline}</h2>
            <p className="body-lg mt-7 max-w-[46rem]">{homePageContent.openResearch.text}</p>
          </Reveal>
          <Reveal delay={0.08} className="flex flex-wrap gap-3 lg:justify-end">
            {homePageContent.openResearch.actions.map((action) => {
              ...
              return external.url ? (
                <ActionLink key={action.linkKey} href={external.url} external variant="secondary">
                  {action.label}
                </ActionLink>
              ) : (
                <PendingAction key={action.linkKey}>{external.pendingLabel}</PendingAction>
              );
            })}
          </Reveal>
        </div>
...
function FeaturedFact({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid gap-2 border-b border-line py-5 sm:grid-cols-[8.5rem_1fr] sm:gap-6">
      <dt className="font-mono text-[0.63rem] tracking-[0.13em] text-muted uppercase">{label}</dt>
