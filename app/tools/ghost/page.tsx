...
const copy = {
  en: {
    intro: {
      eyebrow: "TOOL · GHOST",
      title: "A deception-aware security runtime for autonomous AI agents.",
      description:
    ...
    triad: [
      { k: "ALLOW", v: "Expose a permitted real resource." },
      { k: "DENY", v: "Block access." },
      {
        k: "SHADOW",
        v: "Expose a controlled synthetic resource instead of the corresponding real one.",
      },
    ],
  ...
  de: {
    intro: {
      eyebrow: "TOOL · GHOST",
      title: "Ein deception-aware Security Runtime für autonome KI-Agenten.",
      description:
    ...
    triad: [
      { k: "ALLOW", v: "Eine erlaubte reale Ressource bereitstellen." },
      { k: "DENY", v: "Zugriff blockieren." },
      {
        k: "SHADOW",
        v: "Eine kontrollierte synthetische Ressource statt der realen bereitstellen.",
      },
    ],
    ...
    <>
      ...
      <section className="page-shell pb-[var(--section-space)]">
        <Reveal className="liquid-surface p-7 sm:p-9">
          ...
          <p className="body-copy mt-5 max-w-4xl">{c.baseline}</p>
          <div className="mt-7 flex flex-wrap gap-3">
            <ActionLink href={repo} external variant="primary">
              GitHub
            </ActionLink>
            <ActionLink
              href={`${source}/docs/architecture.md`}
              external
              variant="secondary"
            >
              Architecture
        ...
        <Reveal className="mt-16">
          ...
          <h2 className="display-section mt-6 text-ink">{c.archTitle}</h2>
          <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {c.arch.map((x, i) => (
              <div key={x} className="liquid-card p-5">
                <p className="font-mono text-[0.62rem] text-accent">
                  {String(i + 1).padStart(2, "0")}
                </p>
                <p className="mt-3 text-sm font-medium text-ink">{x}</p>
              </div>
            ))}
          </div>
        </Reveal>
        ...
        <Reveal className="dark-band mt-16 rounded-[2rem] p-8 sm:p-12">
          <p className="eyebrow text-[var(--color-dark-muted)]">FINDINGS</p>
          <h2 className="display-section mt-6 text-white">{c.findingsTitle}</h2>
          <ul className="mt-8 grid gap-5 lg:grid-cols-2">
            {c.findings.map((x) => (
              <li
                key={x}
                className="text-sm leading-7 text-[var(--color-dark-body)]"
              >
                — {x}
              </li>
            ))}
